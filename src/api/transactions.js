...
import { ADGIPSCoinDistributor, ADGIPSCoin, Canteen, FoodNFT, AchievementNFT, } from 'common/constants';

export const getUserTransactions = async (address) => {
  try {
    const response = await Promise.all([
      axios.get(`https://api.ghostnet.tzkt.io/v1/accounts/${address}/operations?target=${ADGIPSCoinDistributor}`),
      //   axios.get(`https://api.ghostnet.tzkt.io/v1/accounts/${address}/operations?target=${ADGIPSCoin}`),
